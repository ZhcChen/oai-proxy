...
Dashboard 和 `/metrics` 会基于 SQLite 记录输出首 token 最低/最高、完整响应最低/最高、
已过滤超时 attempt 总数、响应头超时 attempt 数和首 token 超时 attempt 数。
...
- Dashboard 与 `/metrics` 的耗时 min/max、超时过滤和超时分类统计
