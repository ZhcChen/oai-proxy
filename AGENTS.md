...
- **端口保护：严禁在代码调整、调试或测试过程中占用、重启、停止或干扰 `57999` 端口上的本项目运行实例。** 该端口可能正在被其他项目作为代理服务使用。
- 如需启动本项目进行手动或端到端测试，必须使用 `58000` 端口作为测试端口；禁止使用默认 `cargo run` 直接启动并监听 `57999`。
- 允许运行不绑定真实 `57999` 端口的单元测试、集成测试和编译检查；任何需要真实监听端口的测试都必须先确认使用 `58000`。
